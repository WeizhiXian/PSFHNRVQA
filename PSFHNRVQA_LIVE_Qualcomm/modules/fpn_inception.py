...
class FPNInception(nn.Module):
    ...
    def forward(self, x):
        map00, map01, map02, map03, map04 = self.fpn(x)

        map4 = nn.functional.interpolate(self.head4(map04), scale_factor=8, mode="nearest")
        map3 = nn.functional.interpolate(self.head3(map03), scale_factor=4, mode="nearest")
        map2 = nn.functional.interpolate(self.head2(map02), scale_factor=2, mode="nearest")
        map1 = nn.functional.interpolate(self.head1(map01), scale_factor=1, mode="nearest")

        smoothed = self.smooth(torch.cat([map4, map3, map2, map1], dim=1))
        smoothed = nn.functional.interpolate(smoothed, scale_factor=2, mode="nearest")
        smoothed = self.smooth2(smoothed + map00)
        smoothed = nn.functional.interpolate(smoothed, scale_factor=2, mode="nearest")

        final = self.final(smoothed) # 残差
        res = torch.tanh(final) + x # 修复图

        return final, map00, map01, map02, map03, map04
